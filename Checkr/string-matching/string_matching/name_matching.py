...
def name_match(known_aliases, record_name):
    ...
    if len(names) == 3:
        # Use case 1: For exact match scenario
        if names[0] in alias_first_name and names[1] in alias_middle_name and names[2] in alias_last_name:
            return True

        # Use case 3: If aliases has full middle name and record to match has a single character middle name
        if len(names[1]) == 1:
            if names[0] in alias_first_name and middle_initial(names[1], alias_middle_name) and names[2] in alias_last_name:
                return True

        # Use case 2: If the aliases are stored in single middle character and record name has full middle name
        if names[0] in alias_first_name and names[1][0] in alias_middle_name and names[2] in alias_last_name:
            return True

        # Use case 5: If aliases are stored only in first and last but the record name to match has middle name also
